...
Finally, there's another [helm chart for a flagger-loadtester](helm/infra/service-mesh/flagger-loadtester). This is a Flagger tool that will generate requests to our new version (canary deployemt) during the canary excecution. Without this we would have to manually create this traffic for the canary excecution to work properly. Flagger will read metrics generated by these requests to make decisions on how to proceed with the canary. Without requests there's no metrics, and without metrics Flagger cannot make decisions, effectively rendering the canary deployment pointless.
